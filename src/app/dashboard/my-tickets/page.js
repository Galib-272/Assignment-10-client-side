...
export default function MyTicketsPage() {
  const customFleet = [
    ...
    {
      id: "f_2",
      title: "Ena Transport Hino Luxury (Dhaka - Sylhet)",
      price: 1400,
      available: 10,
    ...
    <div className="max-w-4xl">
      <div className="mb-6">
        <h2 className="text-xl font-extrabold text-gray-900 dark:text-white">
          Active Listed Fleet Cargo
        </h2>
        <p className="text-xs text-gray-400">
          Track and manage booking counts or live review validation pipeline
          updates.
        </p>
      </div>
            ...
            {customFleet.map((fleet) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  <span
                    className={`inline-flex items-center rounded px-2 py-0.5 text-xs font-semibold ${fleet.status === "Approved" ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-400" : "bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-400"}`}
                  >
                    ● {fleet.status}
                  </span>
                </td>
