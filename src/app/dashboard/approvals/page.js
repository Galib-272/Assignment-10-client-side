...
export default function AdminApprovalsPage() {
  ...
  const handleApprove = (id, title) => {
    ...
    alert(`Approved! "${title}" has been successfully injected into the public live inventory list.`);
  };
  ...
  return (
    <div className="max-w-5xl bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 sm:p-8 shadow-sm">
      ...
      <div className="border-b border-gray-100 dark:border-gray-800 pb-4 mb-6">
        <h2 className="text-xl font-extrabold text-gray-900 dark:text-white">Admin Control: Route Approvals</h2>
        <p className="text-xs text-gray-400 mt-1">Audit, approve, or decline system-wide transportation schedules submitted by verified fleet partners.</p>
      </div>
      ...
      {pendingListings.length > 0 ? (
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800/60 text-sm">
              {pendingListings.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50/30 dark:hover:bg-gray-900/10 transition">
                  
                  {/* Route & ID Details */}
                  <td className="py-4 px-4 max-w-xs">
                    <span className="block text-[10px] font-mono text-gray-400">Queue ID: {item.id}</span>
                    <span className="font-bold text-gray-900 dark:text-white block line-clamp-1">{item.title}</span>
                    <span className="text-xs text-gray-500 mt-0.5 block">{item.route}</span>
                  </td>
                  ...
                  <td className="py-4 px-4">
                    <span className="font-extrabold text-gray-900 dark:text-white block">৳{item.price}</span>
                    <span className="text-xs text-gray-400 block">{item.seats} Seats Cap</span>
                  </td>
        ...
        <div className="text-center py-16 border border-dashed border-gray-200 dark:border-gray-800 rounded-xl">
          <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">🎉 Review queue clear! No pending ticket verification tasks remaining.</p>
        </div>
      )}
